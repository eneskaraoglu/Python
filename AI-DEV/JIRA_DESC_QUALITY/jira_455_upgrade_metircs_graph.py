...
def quality_score(row):
    ...
    if row.get('has_code_blocks', False): score += 1  # Eğer has_code_blocks kullanılacaksa
...
quality_counts = df['quality'].value_counts().reindex(['High', 'Medium', 'Low', 'Undefined'], fill_value=0)
...
undefined = df[df['quality'] == 'Undefined']['Creator'].value_counts()

# İki değeri toplayarak her Creator için combined_count hesaplayalım
combined = low_quality.add(undefined, fill_value=0)
...
undefined_count = undefined.reindex(creators, fill_value=0).values
...
bar_width = 0.3  # Üç grup olduğu için uygun genişlik
indices = np.arange(len(creators))

plt.figure(figsize=(12, 6))
plt.bar(indices - bar_width, low_quality_count, width=bar_width, 
        label='Low Kalite JIRA', color='#e74c3c')
plt.bar(indices, undefined_count, width=bar_width, 
        label='Undefined JIRA', color='#95a5a6')
plt.bar(indices + bar_width, total_jira_count, width=bar_width, 
        label='Toplam JIRA', color='#3498db')

plt.xlabel('Creator')
plt.ylabel('JIRA Sayısı')
plt.title('Low + Undefined JIRA Sayısı En Yüksek Olan Creatorlar ve Toplam JIRA Sayıları')
