...
# 1. Low statüsündeki JIRA'ların 'Creator' bazında dağılım grafiği
low_quality_df = df[df['quality'] == 'Low']
low_quality_creator_counts = low_quality_df['Creator'].value_counts()

plt.figure(figsize=(10, 6))
sns.barplot(x=low_quality_creator_counts.index, y=low_quality_creator_counts.values, palette="viridis")
plt.title("Low Kalite JIRA'ların 'Creator' Bazında Dağılımı")
plt.xlabel("Creator")
...
# 2. Toplam JIRA'ların kalite oranları grafiği
quality_counts = df['quality'].value_counts()
# Kalite sıralamasını belirlemek için (High, Medium, Low) sıralı liste oluşturuyoruz.
quality_order = ['High', 'Medium', 'Low']

plt.figure(figsize=(8, 6))
sns.barplot(x=[q for q in quality_order if q in quality_counts.index],
            y=[quality_counts[q] for q in quality_order if q in quality_counts.index],
            palette="coolwarm")
plt.title("Toplam JIRA'ların Kalite Oranları")
plt.xlabel("Kalite")
