...
colors = {'High': '#2ecc71', 'Medium': '#f1c40f', 'Low': '#e74c3c', 'Low': '#e74c3c'}
quality_counts = df['quality'].value_counts().reindex(['High', 'Medium', 'Low', 'DEF'])
...
for creator, count in low_quality_creators.items():
    ...
    # Low kalite JIRA sayılarını elde et (Top 10)
low_quality = df[df['quality'] == 'Low']['Creator'].value_counts().head(10)
# Toplam JIRA sayılarını elde et
total_jira = df['Creator'].value_counts()

# Seçilen kullanıcıları belirleyin
creators = low_quality.index
low_quality_count = low_quality.values
# Sadece seçilen kullanıcıların toplam sayılarını alın
total_jira_count = total_jira.loc[creators].values

# Gruplandırılmış bar grafiği oluşturmak için bar genişliği ve konumlarını ayarlayın
bar_width = 0.35
indices = np.arange(len(creators))

plt.figure(figsize=(12, 6))
plt.bar(indices - bar_width/2, low_quality_count, width=bar_width, 
        label='Low Kalite JIRA', color='#e74c3c')
plt.bar(indices + bar_width/2, total_jira_count, width=bar_width, 
        label='Toplam JIRA', color='#3498db')

plt.xlabel('Creator')
plt.ylabel('JIRA Sayısı')
plt.title('En Çok Düşük Kalite JIRA Oluşturan Kullanıcılar ve Toplam JIRA Sayıları')
